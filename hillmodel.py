...
class HillModel:
    ...
    def save_file(self, file_name="XYZHillData.txt"):
        ...
        for j in range(0, self.nrows):
            for i in range(0, self.ncols):
                txt_file.write(
                    f"{i*self.cellsize}\t{j*self.cellsize}\t{self.point_data[j][i]}\n")
        txt_file.close()
    ...
    def EA2XYZ(self, x_smooth=0, y_smooth=0, z_offset=0, preview="N", save="Y"):
        ...
        for j in range(0, self.nrows):
            for i in range(0, self.ncols):
                if (self.ncols - x_sedge) >= i >= x_sedge and (self.nrows - y_sedge) >= j >= y_sedge:
        ...
        if preview == "Y":
            ...
            scale_max = max([self.ncols * self.cellsize,
                             self.nrows * self.cellsize, self.ht_height])
    ...
    def hill_map(self, domain_length=None, domain_width=None, x_offset=None, y_offset=None, angle=None, inc_size=10, combo=None):
        ...
        self.var_check(domain_length=domain_length, domain_width=domain_width,
                       x_offset=x_offset, y_offset=y_offset, angle=angle)
        ...
        for j in range(0, self.nrows):
            for i in range(0, self.ncols):
                x_prime = x_grid[j][i] * \
                    np.cos(self.alpha) - y_grid[j][i] * np.sin(self.alpha)
                y_prime = x_grid[j][i] * \
                    np.sin(self.alpha) + y_grid[j][i] * np.cos(self.alpha)
        ...
        if combo == "Yes":
            ...
            # Plot contour plot
            fig, ax = plt.subplots()
            cs = ax.contour(t_y_grid, t_x_grid, self.point_data,
                            scale, cmap='YlOrBr', alpha=0.75)
    ...
    def particle_map(self, particle_file, grid_size=5, domain_length=None, domain_width=None, combo=None):
        ...
        for particle in self.trap_particles:
            p_count[int(round(particle[2] // grid_size))
                    ][int(round(particle[1] // grid_size))] += 1
        ...
        if combo == "Yes":
            ...
            fig, ax = plt.subplots()
            cs = ax.contourf(y_grid, x_grid, p_count,
    ...
    def combo_map(self, particle_file, grid_size=5, domain_length=None, domain_width=None, x_offset=None, y_offset=None, angle=None, inc_size=10):
        ...
        self.hill_map(domain_length=self.len, domain_width=self.wid,
                      x_offset=self.dx, y_offset=self.dy, angle=angle, combo="Yes")
        ...
        fig, ax = plt.subplots()
        cs = ax.contour(self.t_y_grid, self.t_x_grid,
                        self.point_data, scale, cmap='YlOrBr', alpha=0.75)
        cs = ax.contourf(self.y_grid, self.x_grid, self.p_count,
                         norm=mpl.colors.LogNorm(), cmap='plasma')
        cbar = fig.colorbar(cs)
    ...
    def landing_bar(self, particle_file, source=None, grid_size=5, domain_length=None, domain_width=None):
        ...
        # Check for previous entry for values
        self.var_check(domain_length=domain_length,
                       domain_width=domain_width, source=source)

        # Calls particle_map method to obtain particle landing locations
        self.particle_map(particle_file, grid_size=grid_size,
                          domain_length=self.len, domain_width=domain_width, combo="Yes")

        # Generate grid for landing zones
        x_co = np.linspace(0, self.len, self.len // grid_size)

        # Counts number of particles that land x distance away from source
        bar_count = [sum(row[i] for row in self.p_count)
                     for i in range(len(self.p_count[0]))]
